...
final class PandorasHybridBoxTests: XCTestCase {
    func test_publicInit_setsAllParametersAndUnderlyingBoxes() {
        ...
        // When
        let box = PandorasHybridBox<String, String>(
            namespace: namespace,
        ...
        let memory = mirror.children.first { $0.label == "memory" }?.value as? PandorasMemoryBox<String, String>
        let disk = mirror.children.first { $0.label == "disk" }?.value as? PandorasDiskBox<String, String>
    ...
    private var memory: PandorasMemoryBox<Key, Value>!
    private var disk: PandorasDiskBox<Key, Value>!
    private var box: PandorasHybridBox<Key, Value>!

    override func setUp() {
        super.setUp()
        cancellables = []
        memory = PandorasMemoryBox<Key, Value>()
        disk = PandorasDiskBox<Key, Value>(namespace: "test")
        box = PandorasHybridBox(
            namespace: "test",
            memory: memory,
            disk: disk,
