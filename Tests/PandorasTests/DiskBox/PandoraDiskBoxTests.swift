...
final class PandorasDiskBoxTests: XCTestCase {
    ...
    var box: PandorasDiskBox<Int, TestCodable>!
    var boxDirectory: URL!

    override func setUpWithError() throws {
        namespace = UUID().uuidString
        box = PandorasDiskBox<Int, TestCodable>(namespace: namespace)
        boxDirectory = PandorasDiskBoxPath.sharedRoot.appendingPathComponent(namespace)
    }
    ...
    func test_givenNonexistentNamespace_whenInit_thenCreatesDirectory() throws {
        ...
        let newDir = PandorasDiskBoxPath.sharedRoot.appendingPathComponent(testNamespace)
        XCTAssertFalse(FileManager.default.fileExists(atPath: newDir.path))
        // When
        let _ = PandorasDiskBox<Int, TestCodable>(namespace: testNamespace)
        // Then
    ...
    func test_givenStaticClearAll_whenSharedRootDoesNotExist_thenNoError() {
        // Given
        let sharedRoot = PandorasDiskBoxPath.sharedRoot
    ...
    func test_givenDifferentNamespaces_whenPutSameKey_thenDataIsIsolated() async {
        ...
        let box1 = PandorasDiskBox<Int, TestCodable>(namespace: ns1)
        let box2 = PandorasDiskBox<Int, TestCodable>(namespace: ns2)
        let key = 99
        ...
        // Cleanup
        let dir1 = PandorasDiskBoxPath.sharedRoot.appendingPathComponent(ns1)
        let dir2 = PandorasDiskBoxPath.sharedRoot.appendingPathComponent(ns2)
    ...
    func test_givenNamespaceWithSlash_whenInit_thenCreatesSafeDirectory() throws {
        ...
        let expectedDir = PandorasDiskBoxPath.sharedRoot.appendingPathComponent(safeNamespace)
        // When
        let _ = PandorasDiskBox<Int, TestCodable>(namespace: namespace)
        // Then
    ...
    func test_givenExpirySet_whenExpired_thenReturnsNilAndRemovesFile() async throws {
        ...
        let expiringBox = PandorasDiskBox<Int, TestCodable>(namespace: expiringNamespace, expiresAfter: 0.05)
        let key = 7
        let value = TestCodable(id: 7, name: "Expiring")
        await expiringBox.put(key: key, value: value)
        let fileURL = PandorasDiskBoxPath.sharedRoot
        ...
        // Cleanup
        let expDir = PandorasDiskBoxPath.sharedRoot.appendingPathComponent(expiringNamespace)
        try? FileManager.default.removeItem(at: expDir)
    }
    func test_givenExpirySet_whenNotExpired_thenReturnsValue() async {
        // Given
        let expiringNamespace = "expire-\(UUID().uuidString)"
        let expiringBox = PandorasDiskBox<Int, TestCodable>(namespace: expiringNamespace, expiresAfter: 10)
        let key = 101
    ...
    func test_givenFileWithExpiredEntry_whenGet_thenRemovesFile() async throws {
        // Given
        let expiringNamespace = "expire-\(UUID().uuidString)"
        let expiringBox = PandorasDiskBox<Int, TestCodable>(namespace: expiringNamespace, expiresAfter: 0.01)
        let key = 70
    ...
    func test_givenMaxSize_whenOverLimit_thenLeastRecentlyUsedEvicted() async {
        ...
        let lruBox = PandorasDiskBox<Int, TestCodable>(namespace: lruNamespace, maxSize: 3)
        ...
        // Cleanup
        let lruDir = PandorasDiskBoxPath.sharedRoot.appendingPathComponent(lruNamespace)
    ...
    func test_givenMaxSizeZero_whenPut_thenLRUNotEnforced() async {
        ...
        let zeroBox = PandorasDiskBox<Int, TestCodable>(namespace: zeroNamespace, maxSize: 0)
        for i in 1...5 {
        ...
        // Cleanup
        let zeroDir = PandorasDiskBoxPath.sharedRoot.appendingPathComponent(zeroNamespace)
    ...
    func test_givenMaxSizeNil_whenPut_thenLRUNotEnforced() async {
        ...
        let nilBox = PandorasDiskBox<Int, TestCodable>(namespace: nilNamespace, maxSize: nil)
        for i in 1...5 {
        ...
        // Cleanup
        let nilDir = PandorasDiskBoxPath.sharedRoot.appendingPathComponent(nilNamespace)
    ...
    func test_givenMaxSize_whenPutMoreThanCapacity_thenLRUEnforced() async {
        // Given
        let lruNamespace = "lru-\(UUID().uuidString)"
        let lruBox = PandorasDiskBox<Int, TestCodable>(namespace: lruNamespace, maxSize: 2)
    ...
    func test_givenFilesWithModificationDates_whenLRU_thenOldestAreEvicted() async throws {
        ...
        let box = PandorasDiskBox<Int, String>(namespace: ns, maxSize: 2)
        // Insert two values
        await box.put(key: 1, value: "first")
        await box.put(key: 2, value: "second")
        let dir = PandorasDiskBoxPath.sharedRoot.appendingPathComponent(ns)
    ...
    func test_givenPutFails_whenEncoderReturnsNil_thenNoFileWritten() async {
        ...
        let stringBox = PandorasDiskBox<String, NonEncodable>(namespace: encNamespace)
        let key = "fail"
        await stringBox.put(key: key, value: NonEncodable(3))
        let url = PandorasDiskBoxPath.sharedRoot
            .appendingPathComponent(encNamespace)
            .appendingPathComponent("fail")
            .appendingPathExtension("box")
        XCTAssertFalse(FileManager.default.fileExists(atPath: url.path))
        // Cleanup
        let encDir = PandorasDiskBoxPath.sharedRoot.appendingPathComponent(encNamespace)
    ...
    func test_givenPutAndGetWithStringKey_thenWorks() async {
        ...
        let stringBox = PandorasDiskBox<String, String>(namespace: dirNamespace)
        let key = "foo"
        let value = "bar"
        // When
        ...
        XCTAssertEqual(result, value)
        // Cleanup
        let strDir = PandorasDiskBoxPath.sharedRoot.appendingPathComponent(dirNamespace)
    ...
    func test_givenClearAll_whenMultipleBoxesUnderSharedRoot_thenAllDataRemoved() async {
        ...
        let box1 = PandorasDiskBox<Int, TestCodable>(namespace: box1Namespace)
        let box2 = PandorasDiskBox<Int, TestCodable>(namespace: box2Namespace)
        await box1.put(key: 1, value: TestCodable(id: 1, name: "A"))
        await box2.put(key: 2, value: TestCodable(id: 2, name: "B"))
        // When
        Pandoras.clearAllDiskData()
